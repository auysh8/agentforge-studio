...
export function CronNode({ id, data, selected }: NodeProps) {
  ...
  const cronExpression = (data?.cronExpression as string) || "0 8 * * *";
    ...
    >
      ...
      <div className="flex items-start justify-between gap-3 p-3.5 pb-2">
        <div className="flex items-start gap-2.5 min-w-0 text-left">
          ...
          >
            <Clock size={18} weight="fill" style={{ color: "var(--family-trigger-icon-color)" }} />
          </div>
          <div className="min-w-0 flex-1 text-left">
            <h3 className="text-[14px] font-medium text-foreground truncate text-left">
              {(data?.label as string) || "Cron Schedule"}
            </h3>
      ...
      <div className="px-3.5 pb-3.5 pt-1 text-left">
        <div className="p-2 rounded-lg bg-muted/40 font-mono text-[11px] text-muted-foreground flex items-center justify-between text-left">
          <div className="flex items-center gap-1.5 truncate">
            ...
            <span className="truncate">{cronExpression}</span>
          </div>
          <span className="text-[9px] bg-background/80 px-1.5 py-0.5 rounded border border-warm-border shrink-0 ml-1 font-semibold uppercase">
            UTC
          </span>
        </div>
      </div>
