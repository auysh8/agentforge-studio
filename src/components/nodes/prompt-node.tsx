...
export function PromptNode({ id, data, selected }: NodeProps) {
  ...
  return (
    <div
      ...
      style={{ borderLeft: "3px solid #3C3489" }}
    >
      {/* Target Handle */}
      <Handle
        type="target"
        position={Position.Left}
        className="!w-3 !h-3 !bg-[#3C3489] !border-2 !border-background hover:!scale-125 transition-transform"
      />
      ...
      <div className="flex items-start justify-between gap-3 p-3.5 pb-2">
        <div className="flex items-start gap-2.5 min-w-0 text-left">
          ...
          >
            <ChatCircle size={18} weight="fill" style={{ color: "#3C3489" }} />
          </div>
          <div className="min-w-0 flex-1 text-left">
            <h3 className="text-[14px] font-medium text-foreground truncate text-left">
              {(data?.label as string) || "Prompt Node"}
            </h3>
      ...
      <div className="px-3.5 pb-3.5 pt-1 text-left">
        <div className="p-2 rounded-lg bg-muted/40 font-mono text-[12px] text-muted-foreground line-clamp-2 leading-relaxed text-left">
          {(data?.prompt as string) || "You are a helpful assistant."}
        </div>
      </div>
