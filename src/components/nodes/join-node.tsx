...
export function JoinNode({ id, data, selected }: NodeProps) {
  ...
  const strategy = (data?.mergeStrategy as string) || "array";
    ...
    >
      ...
      <div className="flex items-start justify-between gap-3 p-3.5 pb-2">
        <div className="flex items-start gap-2.5 min-w-0 text-left">
          ...
          >
            <GitMerge size={18} weight="fill" style={{ color: "var(--family-logic-icon-color)" }} />
          </div>
          <div className="min-w-0 flex-1 text-left">
            <h3 className="text-[14px] font-medium text-foreground truncate text-left">
              {(data?.label as string) || "Join / Merge"}
            </h3>
      ...
      <div className="px-3.5 pb-3.5 pt-1 text-left">
        <div className="p-2 rounded-lg bg-muted/40 flex items-center justify-between text-[11px] text-muted-foreground">
          ...
          <span className="font-mono text-[10px] bg-background/80 px-1.5 py-0.5 rounded border border-warm-border uppercase font-semibold">
            {strategy}
          </span>
        </div>
      </div>
